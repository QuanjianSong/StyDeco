...
def initialize_unet(pretrained_model_name_or_path, rank, return_lora_module_names=False):
...
def initialize_text_encoder(pretrained_model_name_or_path, rank=4, return_lora_module_names=False):
    text_encoder = CLIPTextModel.from_pretrained(pretrained_model_name_or_path, subfolder="text_encoder")
    #
    text_encoder.requires_grad_(False)
    text_encoder.train()
    # breakpoint()
    l_text_encoder_target_modules = [
        "k_proj", "v_proj", "q_proj", "out_proj"
    ]
    text_encoder_lora_config = LoraConfig(r=rank, init_lora_weights="gaussian", target_modules=l_text_encoder_target_modules)
    text_encoder.add_adapter(text_encoder_lora_config, adapter_name="default_text_encoder")
    #
    if return_lora_module_names:
        return text_encoder, l_text_encoder_target_modules
    else:
        return text_encoder


def initialize_vae(pretrained_model_name_or_path, rank=4, return_lora_module_names=False):
...
class CycleGAN_Turbo(torch.nn.Module):
    ...
    @staticmethod
    def get_traininable_params_stage2_all(unet, vae_a2b, vae_b2a, text_encoder_a2b, text_encoder_b2a):
        ...
        # Additional
        for n, p in text_encoder_a2b.named_parameters():
            if "lora" in n and "default" in n:
                # breakpoint()
                assert p.requires_grad
                params_gen.append(p)
        for n, p in text_encoder_b2a.named_parameters():
            if "lora" in n and "default" in n:
                assert p.requires_grad
                params_gen.append(p)
        #
    ...
    def forward(self, x_t, direction=None, caption=None, caption_emb=None):
        ...
        if caption_emb is not None:
            ...
            if direction == 'a2b':
                caption_enc = self.text_encoder_a2b(caption_tokens)[0].detach().clone()
            elif direction == 'b2a':
                caption_enc = self.text_encoder_b2a(caption_tokens)[0].detach().clone()
            else:
