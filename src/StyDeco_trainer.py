...
from cyclegan_turbo import CycleGAN_Turbo, VAE_encode, VAE_decode, initialize_unet, initialize_vae, initialize_text_encoder
from my_utils.training_utils import UnpairedDataset, parse_args_unpaired_training
...
def main(args):
    ...
    #
    text_encoder_a2b, text_encoder_lora_target_modules = initialize_text_encoder(args.pretrained_model_name_or_path, args.lora_rank_text_encoder, return_lora_module_names=True)
    text_encoder_a2b.to(accelerator.device, dtype=weight_dtype)
    text_encoder_b2a = copy.deepcopy(text_encoder_a2b) # 参数不共享，这里就不封装了
    #
    unet, l_modules_unet_encoder, l_modules_unet_decoder, l_modules_unet_others = initialize_unet(args.pretrained_model_name_or_path, args.lora_rank_unet, return_lora_module_names=True)
    ...
    #
    vae_a2b, vae_lora_target_modules = initialize_vae(args.pretrained_model_name_or_path, args.lora_rank_vae, return_lora_module_names=True)
    vae_a2b.to(accelerator.device, dtype=weight_dtype)
    vae_b2a = copy.deepcopy(vae_a2b) # not share
    # two VAE
    ...
    # get params，optimizer，schedule
    params_gen = CycleGAN_Turbo.get_traininable_params_stage2_all(unet, vae_a2b, vae_b2a, text_encoder_a2b, text_encoder_b2a)
    ...
    # get params, optimizer, schedule, optimizer, schedule for disc  |  all trained
    ...
    lr_scheduler_disc = get_scheduler(args.lr_scheduler, optimizer=optimizer_disc,
        ...
        num_cycles=args.lr_num_cycles, power=args.lr_power)
    # train datasets
    dataset_train = UnpairedDataset(dataset_folder=args.dataset_folder,
                                    image_prep="resize_286_randomcrop_256x256_hflip", split="train", tokenizer=tokenizer)
    ...
    #
    unet, vae_enc, vae_dec, text_encoder_a2b, text_encoder_b2a, net_disc_a, net_disc_b = accelerator.prepare(unet, vae_enc, vae_dec, text_encoder_a2b, text_encoder_b2a, net_disc_a, net_disc_b)
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            l_acc = [unet, net_disc_a, net_disc_b, vae_enc, vae_dec, text_encoder_a2b, text_encoder_b2a]
            with accelerator.accumulate(*l_acc):
                img_a = batch["pixel_values_src"].to(dtype=weight_dtype)
                img_b = batch["pixel_values_tgt"].to(dtype=weight_dtype)

                caption_src = batch['caption_src']
                caption_tgt = batch['caption_tgt']
                bsz = img_a.shape[0]
                ...
                """
                # breakpoint()
                # cal embedding, a->b ours
                tokens = tokenizer(caption_tgt, max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt").input_ids
                a2b_embs = text_encoder_a2b(tokens.cuda())[0] # torch.Size([B, 77, 1024])
                a2b_embs = a2b_embs.to(dtype=weight_dtype)
                # cal embedding, b->a ours
                tokens = tokenizer(caption_src, max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt").input_ids
                b2a_embs = text_encoder_b2a(tokens.cuda())[0] # torch.Size([B, 77, 1024])
                b2a_embs = b2a_embs.to(dtype=weight_dtype)
                # A -> fake B -> rec A
                cyc_fake_b = CycleGAN_Turbo.forward_with_networks(img_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, a2b_embs)
                cyc_rec_a = CycleGAN_Turbo.forward_with_networks(cyc_fake_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, b2a_embs)
                ...
                # B -> fake A -> rec B
                cyc_fake_a = CycleGAN_Turbo.forward_with_networks(img_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, b2a_embs)
                cyc_rec_b = CycleGAN_Turbo.forward_with_networks(cyc_fake_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, a2b_embs)
                ...
                b2a_embs = b2a_embs.to(dtype=weight_dtype)
                #
                fake_a = CycleGAN_Turbo.forward_with_networks(img_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, b2a_embs)
                fake_b = CycleGAN_Turbo.forward_with_networks(img_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, a2b_embs)
                ...
                b2a_embs = text_encoder_b2a(tokens.cuda())[0] # torch.Size([B, 77, 1024])
                b2a_embs = b2a_embs.to(dtype=weight_dtype)
                idt_a = CycleGAN_Turbo.forward_with_networks(img_b, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, a2b_embs)
                loss_idt_a = crit_idt(idt_a, img_b) * args.lambda_idt
                loss_idt_a += net_lpips(idt_a, img_b).mean() * args.lambda_idt_lpips
                idt_b = CycleGAN_Turbo.forward_with_networks(img_a, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, b2a_embs)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    # additional
                    eval_text_encoder_a2b = accelerator.unwrap_model(text_encoder_a2b)
                    eval_text_encoder_b2a = accelerator.unwrap_model(text_encoder_b2a)
                    # 
                    eval_net_disc_a = accelerator.unwrap_model(net_disc_a)
                    eval_net_disc_b = accelerator.unwrap_model(net_disc_b)
                    # visualize
                    ...
                    if global_step % args.checkpointing_steps == 1:
                        ...
                        # additional text encoder lora
                        sd["rank_text_encoder"] = args.lora_rank_text_encoder
                        sd["text_encoder_lora_target_modules"] = text_encoder_lora_target_modules
                        sd["sd_text_encoder_a2b"] = eval_text_encoder_a2b.state_dict()
                        sd["sd_text_encoder_b2a"] = eval_text_encoder_b2a.state_dict()
                        # additional disc
                        sd['net_disc_a'] = eval_net_disc_a.state_dict()
                        sd['net_disc_b'] = eval_net_disc_b.state_dict()
                        torch.save(sd, outf)
                        gc.collect()
                        torch.cuda.empty_cache()
